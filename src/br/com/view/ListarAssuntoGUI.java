 ...
 * @author emily_borges
 */
public class ListarAssuntoGUI extends javax.swing.JFrame {
    private JTable tabela = new JTable();
    private DefaultTableModel modelo = new DefaultTableModel();
    
    
    public ListarAssuntoGUI() {
        initComponents();
        criaTabela();
    ...
    private void btNovoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btNovoActionPerformed
        AssuntoGUI gui = new AssuntoGUI(modelo);
        gui.setVisible(true);
    ...
    private void btDeletarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btDeletarActionPerformed
      int linhaSelecionada = tabela.getSelectedRow();
        if(linhaSelecionada >= 0) {
            int idAssunto = (int) tabela.getValueAt(linhaSelecionada, 0);
            AssuntoController cc = new AssuntoController();
            if (cc.remover(idAssunto) == true) {
            modelo.removeRow(linhaSelecionada);
        }
            
        }else{
           JOptionPane.showMessageDialog(null,"É necessario selecionar um assunto.");
        }
    ...
    private void btEditarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btEditarActionPerformed
       int linhaSelecionada = tabela.getSelectedRow();
        if(linhaSelecionada >= 0) {
            int idAssunto = (int) tabela.getValueAt(linhaSelecionada, 0);
           AssuntoGUI gui = new AssuntoGUI (modelo, linhaSelecionada, idAssunto);
            gui.setVisible(true);
    ...
    private void txBuscaNomeActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_txBuscaNomeActionPerformed
        AssuntoController cc = new AssuntoController();
       String Descricao = txBuscaNome.getText();
       modelo.setNumRows(0);
       for (Assunto c: cc.PesquisarAssuntoDescricao(Descricao)) {
           modelo.addRow(new Object[] {c.getCodigo(),c.getDescricao() });
    ...
    private void popularTabela() {
        AssuntoController cc = new AssuntoController();
        for(Assunto c: cc.ListarAssunto()) {
            modelo.addRow(new Object[] { c.getCodigo(), c.getDescricao()});
        }
    }
}
